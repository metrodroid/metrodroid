...
cur.execute('SELECT CompanyName, CompanyName_en FROM StationCode GROUP BY CompanyName, CompanyName_en ORDER BY COUNT(1) DESC')
...
cur.execute('SELECT LineName, LineName_en FROM StationCode GROUP BY LineName, LineName_en ORDER BY COUNT(1) DESC')
i = 1
for row in cur:
  lines[row[0]] = (i, row[1])
  i += 1
...
cur.execute('SELECT AreaCode, LineCode, StationCode, CompanyName, LineName, StationName, StationName_en, Latitude, Longitude FROM StationCode ORDER BY AreaCode, LineCode, StationCode')
station_count = 0
for row in cur:
  ...
  station_id = ((int(row[0]) & 0xff) << 16) + ((int(row[1]) & 0xff) << 8) + (int(row[2]) & 0xff)
