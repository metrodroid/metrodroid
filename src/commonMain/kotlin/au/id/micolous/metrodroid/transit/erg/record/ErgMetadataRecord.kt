...
class ErgMetadataRecord private constructor(val cardSerial: ImmutableByteArray,
                                            val epochDate: Int,
                                            val agency: Int) : ErgRecord(), Parcelable {
    /**
    ...
    override fun toString() = "[ErgMetadataRecord: agency=$agency, serial=$cardSerial, epoch=$epochDate]"
    ...
    companion object {
        fun recordFromBytes(input: ImmutableByteArray): ErgMetadataRecord {
            ...
            val agency = input.byteArrayToInt(2, 2)

            val epochDays = input.byteArrayToInt(5, 2)
            val cardSerial = input.copyOfRange(7, 11)

            return ErgMetadataRecord(cardSerial = cardSerial, epochDate = epochDays,
                    agency = agency)
        }
    }
}
