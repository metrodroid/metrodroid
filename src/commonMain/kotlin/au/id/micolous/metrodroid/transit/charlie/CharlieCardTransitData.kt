...
class CharlieCardTransitData (private val mSerial: Long,
    ...
    override val balance: TransitBalance?
        get() {
            ...
            var expiry = start.plus(Duration.yearsLocal(11)).plus(Duration.daysLocal(-1))
            val candidate = trips.mapNotNull { it.startTimestamp }.maxBy { it.timeInMillis }?.plus(Duration.yearsLocal(2))

            if (candidate != null && candidate.timeInMillis < expiry.timeInMillis) {
                expiry = candidate
            }
