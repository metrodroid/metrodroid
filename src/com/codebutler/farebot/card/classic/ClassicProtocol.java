...
public class ClassicProtocol {
	...
	protected byte[] readBlocks(int sector, int blockIndex, int blockCount, ClassicCardKeys keys) throws Exception {
		...
		int readBlocks = 0;
   	 	for (int index = blockIndex; index < (blockIndex + blockCount); index++) {	
	         ...
	         if (keys == null && sector == 0) {
                 mTech.authenticateSectorWithKeyA(sector, KEY_FIRST_SECTOR);
             } else {
                 ClassicCardKeys.ClassicSectorKey key = keys.keyForSector(sector);
                 if (key.getType().equals(ClassicCardKeys.ClassicSectorKey.TYPE_KEYA)) {
                     mTech.authenticateSectorWithKeyA(sector, key.getKey());
                 } else {
                     mTech.authenticateSectorWithKeyB(sector, key.getKey());
                 }
             }
