...
public abstract class En1545Subscription extends Subscription {
    ...
    @Nullable
    protected Integer getContractTariff() {
        return mParsed.getInt(CONTRACT_TARIFF);
    }

    @Nullable
    protected Integer getContractProvider() {
        return mParsed.getInt(CONTRACT_PROVIDER);
    }
    ...
    public List<ListItem> getInfo() {
        ...
        if (receipt != null && receipt != 0)
            li.add(new ListItem(Localizer.INSTANCE.localizeString(R.string.with_receipt)));
        if (receipt != null && receipt == 0)
            li.add(new ListItem(Localizer.INSTANCE.localizeString(R.string.without_receipt)));
        return li;
    }
}
