...
object LocalizeGenerator {
    ...
    private fun writeCommon(outputDir: File,
                           xmlFile: XMLStringsFile,
                           drawables: List<String>) {
        val writer = makeRFile(outputDir, "commonMain")
        writer.write("/* This file is autogenerated. Do not edit manually.  */\n")
        writer.write("package $pkg\n")
        writer.write("\n")
        writer.write("object Rstring {\n")
        for (string in xmlFile.strings.keys) {
           writer.write("    val $string: StringResource by Rmap.strings\n")
        }
        writer.write("}\n")
        writer.write("\n")
        writer.write("object Rplurals {\n")
        for (string in xmlFile.plurals.keys) {
            writer.write("    val $string: PluralsResource by Rmap.plurals\n")
        }
        writer.write("}\n")
        writer.write("\n")
        writer.write("object Rdrawable {\n")
        for (string in drawables) {
            writer.write("    val $string: DrawableResource by Rmap.drawables\n")
        }
        writer.write("}\n")
        writer.close()
    }

    private fun writeMap(writer: OutputStreamWriter, varName: String,
                         typeName: String, keys: Iterable<String>,
                         transform: (name: String, type: String) -> String) {
        writer.write("    actual val $varName = mapOf(\n")
        for ((idx, string) in keys.withIndex()) {
            if (idx != 0)
                writer.write(",\n")
            writer.write("        \"$string\" to " + transform(string, typeName))
        }
        writer.write("\n    )\n")
    ...
    private fun writeRfile(outputDir: File, flavour: String,
        ...
        writer.write("package $pkg\n")
        writer.write("\n")
        writer.write("actual object Rmap {\n")
        writeMap(writer, "strings", "string", xmlFile.strings.keys, transform)
        writeMap(writer, "plurals", "plurals", xmlFile.plurals.keys, transform)
        writeMap(writer, "drawables", "drawable", drawables, transform)
        writer.write("}\n")
    ...
    fun generateLocalize(outputDir: File, stringsFile: File, drawablesDirs: List<File>) {
        ...
        writeCommon(outputDir, xml, drawables)
        writeRfile(outputDir, "androidMain", xml, drawables) { name, type -> "$androidR.$type.$name" }
    ...
    private fun writeFallbackRFile(outputDir: File, flavour: String, xmlStrings: XMLStringsFile, drawables: List<String>) {
        writeRfile(outputDir, flavour, xmlStrings, drawables) { name, type ->
            when (type) {
                "string" -> "${typeName(type)}(\"$name\", \"${escape(xmlStrings.strings[name]!!)}\")"
                "plurals" -> "${typeName(type)}(\"$name\", \"${escape(xmlStrings.plurals[name]!!["one"]!!)}\", \"${escape(xmlStrings.plurals[name]!!["other"]!!)}\")"
